...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  switch (rootOwner) {
    case deployer:
      const tx2 = await root.connect(await ethers.getSigner(deployer)).transferOwnership(owner)
      console.log(`Transferring root ownership to final owner (tx: ${tx2.hash})...`)
      await tx2.wait()
    case owner:
      const controller = await root.controllers(owner);
      if (!controller) {
        const tx2 = await root.connect(await ethers.getSigner(owner)).setController(owner, true)
