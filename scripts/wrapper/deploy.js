...
const onsAppSourcePath = '../ons-app/cypress.env.json'
...
async function main() {
  ...
  if(network.name === 'localhost'){
    ...
    registryAddress = addresses.onsAddress
    registrarAddress = addresses.baseRegistrarAddress
    metadataUrl = 'http://localhost:8080/name/0x{id}'
    if(!(addresses.onsAddress && addresses.baseRegistrarAddress)){
      throw('please run yarn preTest on ../ons-app')
    }  
  }else{
